 ...
 * Modified by: Brandon Gomez (brlgomez@ucsc.edu), 11/9/12
 *
 * CMP 12A/L: Fall 2013
 * Lab Assignment #6
 * 							
...
class ImageProc {
		...
		do {
			...
			case 'E':
				...
				if(fileRead) {
					...
					System.out.println("The image has been edge detected.");
					imageModified = true;
				}
				else {
					System.out.println("Sorry; there is no image to edge detect.");
				}
	...
	public static void writeOutFile(PrintWriter newFile, int[][] pictureArray) {
		...
		for(int i = 0; i < pictureArray.length; i++){
			for(int j = 0; j < pictureArray[i].length; j++){
				newFile.write(pictureArray[i][j] + " ");
			}
			newFile.println();
		}
		newFile.close();
	...
	public static int[][] edge(int[][] a) {
		...
		for(row = 0; row < height; row++)
			for(col = 0; col < width; col++)
				h[row][col] = h[row][col]*h[row][col];
		
		for(row = 0; row < height; row++)
			for(col = 0; col < width; col++)
				v[row][col] = v[row][col]*v[row][col];
		
		for(row = 0; row < height; row++)
			for(col = 0; col < width; col++){
				f[row][col] = (v[row][col]+h[row][col])/9;
			}
		return f;
	}
